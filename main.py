...
for exp_num in ["exp_1", "exp_2", "exp_3", "exp_4"]:
    ...
    for poly_degree in polynomial_degree_list:
        ...
        model = NetAnyFunctional(input_size=1, num_layers=num_layers, hidden_size =hidden_size, device=device,
                                 functional = functional, poly_degree = poly_degree, fourier_degree = None, poly_mu_l_degree = None,
        ...
        # # #Pretrain model
        pretrainer(model, hifi_pretrain_data, lofi_pretrain_data, valid_data, "pretrain", "model", device=device, n_iter=pre_train_iter, learning_rate=lr, scheduler_type="StepLR",
                   save_freq=1000, weight_decay=0, valid_freq=valid_freq, num_workers=num_workers, batch_size=batch_size)
        ...
        for i, (n_hifi, var_0) in enumerate(zip(n_H_store, var_0_store)):
            ...
            directory = "{:s}/{:s}/degree_{:s}/r_{:s}".format(exp_num, functional, str(poly_degree), str(n_hifi / 4)).replace('.', '_')
            os.makedirs(directory, exist_ok=True)
            run_name = "1"
            experiment_name = directory

            # Normal trainer model, hifi_data, lofi_data, valid_data, experiment_name, model_save_name, var_0, alpha_0, beta_0, n_count,
            run_id = trainer(model, H_train_data, L_train_data, D_train_data, valid_data, experiment_name, "model", hyperparameters= [var_0, alpha_0,
                             beta_0, n_count], run_name=run_name, device=device, n_iter=train_n_iter,
                             learning_rate=lr, scheduler_type="StepLR", save_freq=1000, weight_decay=0,
    ...
    for poly_on_mu_l_degree in polynomial_on_mu_l_degree_list:
        ...
        model = NetAnyFunctional(input_size=1, num_layers= num_layers, hidden_size=hidden_size, device=device, functional = functional ,poly_degree = None, fourier_degree = None, poly_mu_l_degree = poly_on_mu_l_degree, activation_func =activation_func).to(device)
        ...
        for i, (n_hifi, var_0) in enumerate(zip(n_H_store, var_0_store)):
            ...
            n_count = [n_hifi, n_lofi]  # Hifi and Lofi Prior
            # Create results directory
            directory = "{:s}/{:s}/degree_{:s}/r_{:s}".format(exp_num, functional, str(poly_on_mu_l_degree), str(n_hifi / 4)).replace('.', '_')
            os.makedirs(directory, exist_ok=True)
            run_name = "1"
            experiment_name = directory
    ...
    for fourier_degree in fourier_degree_list:
        ...
        model = NetAnyFunctional(input_size=1, num_layers= num_layers, hidden_size=hidden_size, device=device, functional = functional ,poly_degree = None, fourier_degree = fourier_degree, poly_mu_l_degree = None, activation_func =activation_func).to(device)
        ...
        for i, (n_hifi, var_0) in enumerate(zip(n_H_store, var_0_store)):
            ...
            n_count = [n_hifi, n_lofi]  # Hifi and Lofi Prior
            # Create results directory
            directory = "{:s}/{:s}/degree_{:s}/r_{:s}".format(exp_num,functional, str(fourier_degree), str(n_hifi/4)).replace('.', '_')
            os.makedirs(directory, exist_ok=True)
            run_name = "1"
            experiment_name = directory
    ...
    # # Periodic Functional
    functional = 'periodic'
    model = NetAnyFunctional(input_size=1, num_layers=num_layers, hidden_size=hidden_size, device=device,functional=functional, poly_degree=None, fourier_degree=None, poly_mu_l_degree=None, activation_func=activation_func).to(device)
    ...
    # # #Pretrain model
    pretrainer(model, hifi_pretrain_data, lofi_pretrain_data, valid_data, "pretrain", "model", device=device, n_iter=pre_train_iter, learning_rate=lr, scheduler_type="StepLR", save_freq=1000, weight_decay=0, valid_freq=valid_freq, num_workers=num_workers, batch_size=batch_size)
    torch.save(model, pre_train_lofi_directory + "/mse_model")
    ...
    for i, (n_hifi, var_0) in enumerate(zip(n_H_store, var_0_store)):
        ...
        n_count = [n_hifi, n_lofi]  # Hifi and Lofi Prior
        # Create results directory
        directory = "{:s}/{:s}/r_{:s}".format(exp_num, functional, str(n_hifi/4)).replace('.', '_')
        os.makedirs(directory, exist_ok=True)
        run_name = "1"
        experiment_name = directory
        ...
        run_id = trainer(model, H_train_data, L_train_data, D_train_data, valid_data, experiment_name, "model",
                         hyperparameters=[var_0, alpha_0,beta_0, n_count], run_name=run_name, device=device, n_iter=train_n_iter,
                         learning_rate=lr, scheduler_type="StepLR", save_freq=1000, weight_decay=0,
                         valid_freq=valid_freq, num_workers=num_workers, batch_size=batch_size)
        torch.save(model, directory + "/final")
    ...
    for poly_degree in polynomial_degree_list_overlap:
        for poly_on_mu_l_degree in polynomial_on_mu_l_degree_list_overlap:
            ...
            for i, (n_hifi, var_0) in enumerate(zip(n_H_store, var_0_store)):
                model = torch.load(pre_train_lofi_directory + "/mse_model", map_location=device)
                n_count = [n_hifi, n_lofi]  # Hifi and Lofi Prior
                directory = "{:s}/{:s}/degree_{:s}_mu_l_degree_{:s}/r_{:s}".format(exp_num, functional, str(poly_degree), str(poly_on_mu_l_degree), str(n_hifi/4)).replace('.', '_')
                os.makedirs(directory, exist_ok=True)
                run_name = "1"
                experiment_name = directory
    ...
    for poly_degree in polynomial_degree_list_overlap:
        ...
        model = NetAnyFunctional(input_size=1, num_layers=num_layers, hidden_size=hidden_size, device=device,functional=functional, poly_degree=poly_degree, fourier_degree=None, poly_mu_l_degree=None,activation_func=activation_func).to(device)
        pre_train_lofi_directory = "{:s}/{:s}/degree_{:s}".format(exp_num, functional, str(poly_degree)).replace('.', '_')
        os.makedirs(pre_train_lofi_directory, exist_ok=True)
        run_name = "1"
        experiment_name = pre_train_lofi_directory
